...
def roomdetails(request):
    return HttpResponse(render(request,'roomdetails.html'))
    ...
    if request.method =="POST":
        try:
            ...
            if len(room) == 0:
                messages.warning(request,"Sorry No Rooms Are Available on this time period")
            data = {'rooms':room,'all_location':all_location,'flag':True}
            response = render(request,'result.html',data)
            return HttpResponse(response)
        except Exception as e:
            messages.error(request,e)
            response = render(request,'result.html',{'all_location':all_location})
            return HttpResponse(response)
...
def user_sign_up(request):
    if request.method =="POST":
        ...
        new_user = User.objects.create_user(username=user_name,password=password1,first_name=first_name,last_name=last_name)
